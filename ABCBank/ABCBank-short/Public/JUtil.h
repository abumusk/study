...
public:
	...
	static uint16 NotworkToHost16(uint16 net16)
	{
		if (IsBigEndian())
			return net16;
	...
	static uint32 NotworkToHost32(uint32 net32)
	{
		if (IsBigEndian())
			return net32;
	...
	static uint64 NotworkToHost64(uint64 net64)
	{
		if (IsBigEndian())
			return net64;
...
private:
	static bool IsBigEndian()		//判定主机是大端字节序还是小端字节序，大端返回真 大端字节序是高位数据放在低位地址
	{
