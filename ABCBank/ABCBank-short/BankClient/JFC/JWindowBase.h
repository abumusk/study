...
public:
	...
	//绘制文件(x, y相对于本窗口的位置，并不是相对整个控制台的位置)
...
private:
	...
	public:
		...
		//把缓冲区内容刷新到屏幕
		//GetStdHandle 获取句柄，标准输入，标准输出，标准错误
		//STD_OUTPUT_HANDLE 标准输出
		//WriteConsoleOutput 刷新屏幕
		//
		void refresh(SHORT x, SHORT y, SHORT width, SHORT height)
		{
			COORD size = {80, 25};	//整个缓冲区大小
			COORD coord = {x, y};	//要缓刷的坐标
			JRECT rect = {x, y, x+width-1, y+height-1 };	//要刷新的区域左上角和右下角
														//rect输入输出参数，输出的是实际写入的到屏幕的大小
...
public:
	static ScreenBuffer sb_;	//是整个类所共有的，不属于任一对象的
