...
void ReceiptForm::OnKeyEvent(JEvent* e)
{
	...
	if (key == KEY_ENTER && e->GetSender() == btnReturn_)
	{
		...
		{
		case RFT_OPEN_ACCOUNT:
			//dynamic_cast<OpenAccountForm*>(form);
			form = Singleton<FormManager>::Instance().Get("OpenAccountForm");
			break;
		case RFT_DEPOSIT:
			//dynamic_cast<DepositForm*>(form);
			form = Singleton<FormManager>::Instance().Get("DepositForm");
			break;
		case RFT_WITHDRAW:
			//dynamic_cast<WithdrawalForm*>(form);
			form = Singleton<FormManager>::Instance().Get("WithdrawalForm");
			break;
		case RFT_TRANSFER:
			//dynamic_cast<TransferForm*>(form);
			form = Singleton<FormManager>::Instance().Get("TransferForm");
			break;
		case RFT_BALANCE_INQUIRY:
			//dynamic_cast<MainMenuForm*>(form);
			form = Singleton<FormManager>::Instance().Get("BalanceInquiryForm");
			break;
		case RFT_CLOSE_ACCOUNT:
			//dynamic_cast<MainMenuForm*>(form);
			form = Singleton<FormManager>::Instance().Get("CloseAccountForm");
			break;
		}
