...
int BankService::OpenAccount(Account& account)
{
	...
	try
	{
		...
		ss<<"insert into bank_account values(null, '" <<
			...
			account.pass <<"', '" <<
			account.id <<"', " <<
			setprecision(2) <<setiosflags(ios::fixed) <<account.balance <<", now(), " <<	//setiosflags(ios::fixed)小数点表示 setprecision(2)浮点数精确两位
			0 <<");" ;
...
//销户
int BankService::CloseAccount(Account& account, double& interst)
{
	/*
	...
	select max(trans_id) from trans where account_id = 3 group by date_format(trans_date, '%Y-%m-%d')
	)
	...
	*/
	...
	try
	{
		db.Open(server.GetServerIp().c_str(),
			server.GetDbUser().c_str(),
			server.GetDbPass().c_str(),
			server.GetDbName().c_str(),
			server.GetDbServerPort());
		...
		rs.Clear();
		rs = db.QuerySQL(ss.str().c_str());
		interst = Convert::StringToDouble(rs.GetItem(0, "interest"));
...
{
	/*
	select balance form bank_account where account_id = 5 fro update;	//加update锁
	update bank_account set balance=balance+100 where account_id = 5;
	insert into tans values(....);
	*/
...
//取款
int BankService::Withdrawal(Account& account)
{
...
int BankService::Transfer(Account& account, const string& otherAccountId)
{
	...
	try
	{
		...
		double otherbalance = Convert::StringToDouble(rs.GetItem(0, "balance"));
		...
		ss<<"insert into trans values(null, " <<
			...
			", "<<4 <<", " <<
			setprecision(2) <<setiosflags(ios::fixed) <<account.balance <<", " <<
			setprecision(2) <<setiosflags(ios::fixed) <<otherbalance+account.balance <<
			", now());";
...
{
	/*
	select a.account_id, a.other_account_id, b.name, a.monty, a.balance, a.trans_date
	from
...
int BankService::QueryHistoryBill(list<TransDetail>& result, int page, const string& begin, const string& end)
{
	...
	from
	(select * from trans where date_format(trans_date, '%Y-%m-%d') = '2014-0-03') a, abstract b
	where a.abstract_id = b.abstract_id order by trans_date;
	*/
...
int BankService::QueryAccountHistoryBill(list<TransDetail>& result, int page, const string& accountId, const string& begin, const string& end)
{
	...
	between '2013-01-10' and '2013-08-2') a, abstract b
	...
	*/
	...
	try
	{
		...
		int total = Convert::StringToInt(rs.GetItem(0, "tatal"));
