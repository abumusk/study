...
BankSession::BankSession(std::auto_ptr<Socket>& socket) : socket_(socket), isDeal_(false)
{
...
void BankSession::Recv()
{
	...
	uint16 cmd = Endian::NotworkToHost16(requestPack_->head.cmd);
	uint16 len = Endian::NotworkToHost16(requestPack_->head.len);
