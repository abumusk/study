...
std::vector<string> split(const string& str)
{
  ...
  while (pos != string::npos)
  {
    ...
    start = pos+1;
    pos = str.find('/', start);	//从start开始查找'/'
  }

  if (start < str.length())	//说明最后一个字符不是'/'
  {
    result.push_back(str.substr(start));	//从start开始切割
  }
      ...
      processInspector_(new ProcessInspector)
{
  ...
  //这样子做法是为了防止竞态问题
  //如果直接调用start,(当前线程不是loop所属的IO线程，是主线程)那么有可能，当前构造函数还没有返回，
  //HttpServer所在的IO线程可能已经收到了http客户端的请求了(因为这个时候httpServer已启动)，那么就会回调
  //Inspector::onRequest,而这个时候构造函数还没有返回，也就是说对象还没有完全构造好
...
void Inspector::onRequest(const HttpRequest& req, HttpResponse* resp)
{
  ...
  //以"/"进行分割，将得到的字符串保存在result中
    ...
    if (result.size() == 0)
    {
    	//这种情况是错误的，因为ok仍然是false
    }
    else if (result.size() == 1)
    {
    //只有mudule,没有 command也是错的，因为ok仍为false
    ...
    else
    {
      ...
      if (commListI != commands_.end())
      {
        ...
        if (it != commList.end())
        {
          ArgList args(result.begin()+2, result.end());	//传递给回调函数参数表
          if (it->second)
          {
            resp->setStatusCode(HttpResponse::k200Ok);
            resp->setStatusMessage("OK");
