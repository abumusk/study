...
class TcpClient : boost::noncopyable
{
 ...
 private:
  ...
  ConnectorPtr connector_; // avoid revealing Connector	用于发起主动连接
  ...
  bool retry_;   // atmoic 重连，是指连接建立之后双断开的时候是否得连
  bool connect_; // atomic	是否连接
  // always in loop thread
  int nextConnId_;	//name + nextConnId_用于标识一个连接
  mutable MutexLock mutex_;
  TcpConnectionPtr connection_; // @BuardedBy mutex_ Connector连接成功以后，得到一个TcpConnection
};
