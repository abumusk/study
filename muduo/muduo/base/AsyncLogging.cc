...
void AsyncLogging::append(const char* logline, int len)
{
  ...
  {
  //当前缓冲区已满，将当前缓冲添加到待写入文件的已填满的缓冲区列表
...
void AsyncLogging::threadFunc()
{
  ...
  while (running_)
  {
...

    {
      ...
      if (!nextBuffer_)
      {
        nextBuffer_ = boost::ptr_container::move(newBuffer2);//确保前端如终有一个预备buffer可供调配，
	...
	//消息堆积
	//前端陷入死循环，拼命发送日志消息，超过后端的处理能，这就是典型的生产速度
    ...
    if (buffersToWrite.size() > 2)
    {
      ...
      buffersToWrite.resize(2);	//信保存两个buffer,用于newBuffer1与newBuffer2
    }
