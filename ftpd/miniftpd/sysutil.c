...
int tcp_client(unsigned short port)
{
	...
	{
		int no = 1;
		if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (const char*)&no, sizeof(no)) < 0)
...
const char* statbuf_get_perms(struct stat *sbuf)
{
	...
	if (mode & S_ISGID)
	{
		perms[3] = (perms[6] == 'x') ? 's' : 'S';
	}

	if (mode & S_ISGID)
	{
		perms[3] = (perms[9] == 'x') ? 's' : 'S';
	}
...
const char* statbuf_get_date(struct stat *sbuf)
{
	...
	//用秒时间来获取tm结构体，因strftime需要的是tm的结构体
...
{
	time_t secs = (time_t)seconds;				//获取整数部分
	double fractional = seconds - (double)secs;//获取小数部分
	...
	do
	{
		 ret = nanosleep(&ts, &ts);	//第一个参数是要睡眠的时间，第二个参数是剩余时间，可能会被信号打断
	}
