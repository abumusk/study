...
void check_linits(session_t *sess);

void handle_sigchle(int sig);

unsigned int hash_ip_count(void *ip);
...
const char *tunable_listen_address;
	*/
	...
	if (getuid() != 0)
	{
		fprintf(stderr, "miniftpd: must be started ar root\n");
		exit(EXIT_FAILURE);
	}
/*
...
*/
	...
	while (1)
	{
		...
		sess.num_this_ip = hash_ip_count(&ip);
...
unsigned int hash_ip_count(void *ip)
{
	...
	//即该ip对应的连接数要加1，如果这个表项还不存在，
	//要在表中添加一条记录，并且将ip对应的连接数置1。
