...
void limit_rate(session_t *sess, int bytes_transfered, int is_upload)
{
	...
	nano_sleep(pause_time);				//不能使用sleep，因为sleep的输入参数是整数，并且sleep内部使用SIGALRM信号来实现，这样alarm和sleep不能同时使用，alarm实现空闲断开链接
...
static void do_pass(session_t *sess)
{
	...
	//将明文密码进行加密 crypt(参数1明文，参数2种子，用影子文件中已经加密的密码)返回加密过的密码
	...
	//注册当前进程可接受SIGURG信号，并开启当前进程有能力接受SIGURG信号，可接受带外数据（紧争数据）
